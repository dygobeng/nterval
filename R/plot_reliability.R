...
plot_reliability <- function(n,
  ...
  plot_data <- data.frame(coverage = reliability_list[["sim_coverage"]])
  break_values <- c(all_break_values[which(all_break_values > min(plot_data$coverage))], 1)
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::geom_histogram(
      subset(
        plot_data,
        coverage >= prox_lo & coverage <= prox_hi
      ),
      mapping = ggplot2::aes(x = coverage),
      breaks = break_values,
